          ...
          <div class="content-wrapper">
            <div class="page-header">
              <h3 class="page-title">
                ...
                </span> <a href="{{url('/home')}}" style="color: black;text-decoration: none;">Dashboard</a> / Extension Service(s) Records
              </h3>
                    ...
                    <table class="table table-bordered">
                      ...
                      <tbody>
                        <tr>
                          @foreach ($exss as $exs)
                          <td> {{ $exs->id }} </td>                      
                          <td> {{ $exs->product_or_service }} </td>
                          <td> {{ $exs->type }} </td>  
                          <td> {{ $exs->date }} at {{ $exs->time }} </td>
                          <td> {{ $exs->payable_amount }} </td>
                          <td><form action="{{ route('cart.add') }}" method="POST">
                            @csrf
                            <input type="hidden" name="product_or_service_id" value="{{ $exs->id }}">
                            <input type="hidden" name="product_type" value="Extension Service">
                            <input type="hidden" name="payable_amount" value="{{ $exs->payable_amount }}" required>
